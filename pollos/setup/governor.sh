...
# edit the installed copy at /usr/local/sbin/governor.sh and run it.
#
...
case "${1:-}" in
  ...
  install)
    ...
    cmp -s "$0" /usr/local/sbin/governor.sh || install -m 0755 "$0" /usr/local/sbin/governor.sh
...
Type=oneshot
RemainAfterExit=yes
ExecStart=/usr/local/sbin/governor.sh
